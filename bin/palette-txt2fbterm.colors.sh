...
cat ./temp.1 >> "./${BN}.fbterm.${CN}colors.0"

### the end
echo "fi" >> "./${BN}.fbterm.${CN}colors.0"

cat "./${BN}.fbterm.${CN}colors.0" | sed 's/::/\"/g' > "${BN}.fbterm.${CN}colors.sh"
rm -f ./temp.*
rm -f "./${BN}.fbterm.${CN}colors.0"

echo "$CN colors written to '${BN}.fbterm.${CN}colors.sh'"
